...
- **Strip min height:** `--breadcrumb-slot-min-height` (default `var(--spacing-3xl)`, 48px)
- **Strip border (optional):** `--breadcrumb-slot-border-bottom` (default `none`; set to a stroke token when a divider is required)
