# Deduplication Roadmap

Roadmap to remove duplicated ownership and keep one source of truth.

## Guiding principle

If a component has a stable Aria/token implementation path, this repo must not keep a second active definition for the same component.

## Wave 1 (priority): Navbar

### Why first

- High visibility shell component.
- Currently duplicated in local specs/docs/rules.

### Local files to demote from official ownership

- `component-specs/navbar.json`
- `docs/components/logos-and-navbar.md`
- `.cursor/rules/navbar-logos.mdc`
- Any template rule/doc that hardcodes local navbar ownership.

### Transition mode

1. Keep one canonical navbar contract across rule/doc/spec.
2. Mark hybrid Aria model as the default implementation source.
3. Avoid generating multiple navbar variants with conflicting ownership.

## Wave 2 (high priority): Menu and Select

`Menu` and `Select` are active normalization targets to converge on Aria/tokens with optional wrappers.

### Local files to demote from official ownership

- `component-specs/menu.json`
- `component-specs/select.json`
- `docs/components/menu-and-select.md`
- Any rule/doc that defaults to local ownership for menu/select

### Transition mode

1. Keep local menu/select artifacts as operational contracts.
2. Mark AriaBase as default implementation source.
3. Avoid creating duplicate variants disconnected from token/foundation rules.

## Wave 3: Other overlapping components

After each integration/update cycle, evaluate overlap for:

- Drawer
- Card/button/icon patterns currently covered by `titan-aria`

If duplicated implementations exist, keep one canonical path and deprecate the rest.

## Deprecation workflow

1. **Detect overlap** (inventory update).
2. **Flag local artifact** as deprecated fallback.
3. **Update references** in docs/rules to AriaBase or TitanAriaWrapper.
4. **Migrate consuming code** to the canonical Aria/token path.
5. **Remove fallback** once no consumers remain.

## Exit criteria per component

A local fallback can be removed when:

- Design/UX stakeholders accept parity.
- Accessibility parity is verified.
- No active consumer depends on the local fallback.
