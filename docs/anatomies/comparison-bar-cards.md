...
- **Table sorting:** Same pattern as all card tables: **sortDescriptor** + **onSortChange** on `TitanTable`; sortable columns use `TitanColumn allowsSorting`; sorted header gets **background color** (`--table-slot-header-sorted-bg`).
- **Components:** `TitanCardGrid` + `TitanCard`, `TitanTable` + `TitanTableHeader`, `TitanColumn`, `TitanTableBody`, `TitanRow`, `TitanCell`, `TitanProgressBar`, `TitanDrawer` (triggerLabel + triggerClassName + triggerIcon for tertiary “Show full table”), `TitanButton` (tertiary for “Read more”, “Download”).
