...
import type { TitanClusterData, TitanClusterLink, TitanClusterNode } from './types'
...
export function buildTitanClusterMockData(
  groupCount: number,
  width: number,
  height: number,
  nodeCount = TITAN_CLUSTER_DEFAULT_NODE_COUNT,
): TitanClusterData {
  const nodes: TitanClusterNode[] = []
  const links: TitanClusterLink[] = []
  ...
  nodes.forEach((source) => {
    if (Math.random() > 0.9) {
      ...
      if (differentGroupNodes.length > 0) {
        const target =
          differentGroupNodes[Math.floor(Math.random() * differentGroupNodes.length)]
