...
interface TitanClusterGraphCanvasProps {
  width: number
  height: number
  data: TitanClusterData
  onNodeClick: (node: TitanClusterNode) => void
  showLabels: boolean
  groupCount: number
  segmentLabels?: string[]
}

export function TitanClusterGraphCanvas({
  width,
  height,
  data,
  onNodeClick,
  ...
  useEffect(() => {
    ...
    const groupFoci: TitanClusterGroupCenter[] = Array.from(
      { length: groupCount },
      (_, i) => {
    ...
    const simulation = d3
      .forceSimulation<TitanClusterNode>(data.nodes)
      .alphaDecay(0.08)
      .velocityDecay(0.35)
      .alphaMin(0.001)
      .force(
        'link',
        d3
          .forceLink<TitanClusterNode, TitanClusterLink>(data.links)
          .id((d) => d.id)
      ...
      .force(
        'collide',
        d3.forceCollide<TitanClusterNode>((d) => d.radius + 1).strength(0.8),
      )
      ...
      .force(
        'x',
        d3
          .forceX<TitanClusterNode>((d) => (groupFoci[d.group] || groupFoci[0]).x)
          .strength(0.28),
      )
      .force(
        'y',
        d3
          .forceY<TitanClusterNode>((d) => (groupFoci[d.group] || groupFoci[0]).y)
          .strength(0.28),
      )
    ...
    const node = svg
      ...
      .join('circle')
      .attr('r', (d) => d.radius)
      .attr('fill', (d) => TITAN_CLUSTER_COLORS[d.group] || TITAN_CLUSTER_COLORS[0])
      .attr('stroke', '#fff')
      .attr('stroke-width', 0.5)
      ...
      .call(
        d3
          .drag<SVGCircleElement, TitanClusterNode>()
    ...
    simulation.on('tick', () => {
      link
        .attr('x1', (d) => (d.source as TitanClusterNode).x!)
        .attr('y1', (d) => (d.source as TitanClusterNode).y!)
        .attr('x2', (d) => (d.target as TitanClusterNode).x!)
        .attr('y2', (d) => (d.target as TitanClusterNode).y!)
    ...
    function dragstarted(
      _event: d3.D3DragEvent<SVGCircleElement, TitanClusterNode, TitanClusterNode>,
      d: TitanClusterNode,
    ) {
      d.fx = d.x
      d.fy = d.y
    ...
    function dragged(
      event: d3.D3DragEvent<SVGCircleElement, TitanClusterNode, TitanClusterNode>,
      d: TitanClusterNode,
    ) {
